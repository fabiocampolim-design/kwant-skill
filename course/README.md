...
Slides, a printable handout and lecturer notes for a course taught from
`Kwant_Theory_and_Practice.ipynb`. Everything is generated from one content
source and from the executed notebook; nothing here is drawn or written twice.
...
| **Figures** (all 68 of the notebook, numbered as there) | `figures/fig-NN.png` + `figures/figures.json` | extracted from the executed notebook |
...
carries its complete notebook caption plus its section and cell. If reveal.js
...
dimensions · Close. Stops 8–9 are Part II of the notebook and make a second
lecture on their own.

## Rebuilding

```
python course/build_course.py [--outdir DIR] [--notebook FILE] [--skip-handout] [--quiet]
```

Extracts every `image/png` output of the notebook into `figures/` (so a
figure exists here only if a notebook cell produced it), renders
...
Re-execute the notebook first if you changed it (`AGENTS.md`, Commands).

Figures are referenced by their notebook number; `figures/figures.json` maps
each number to the cell that made it and its caption.
